...
- **The Assistant is rule-based, not an LLM.** It matches scheme names, departments,
  and occupations by keyword/fuzzy-match against the catalogue, and answers roadmap
  questions from a fixed script — in English or Hindi. That is deliberate: it cannot
  hallucinate a scheme or an eligibility rule that doesn't exist, but it also can't
  handle open-ended phrasing an LLM would. Swapping in an LLM later is a drop-in
  replacement for `chatbot.respond()`, not a redesign.
